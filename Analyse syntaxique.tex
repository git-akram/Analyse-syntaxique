...
\hline
\prec\prec...\succ\succ & Définie une action sémantique & \prec\prec printf("\%S",a);\succ\succ \\
\hline				
\left(...) & Régle & ("int" | ID | storage\_class) \\
\hline
\left(...)* & Closure & ID("\," ID)* \\
\hline
\left(...)+ & Positive Closure & slist : ( stat | SEMICOLON ) \+ \; \\
\hline
\left\{...\} & Optionnel & \{ELSE stat\} \\
\hline
\prec\prec...\succ\succ? & Prédicat sémantique & type : \prec\prec is\_Type(str)\succ\succ ? ID; \\
\hline
\left(...)? & Prédicat syntaxique & ((list EQ))? list EQ list | list ) \\
\hline
\end{tabular}
