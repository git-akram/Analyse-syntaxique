...
\newpage
\begin{abstract}
Le processus de compilation passe par une série d’étapes afin de générer un programme cible à partir d’un programme source. Il passe par une série d’étapes dont l’une qui nous intéresse est l’analyse syntaxique. Son principe est de recevoir une série d’unités lexicales formé à l’aide de l’analyse lexicale puis générer un arbre de syntaxe abstraite à base d’une grammaire qui définit le langage traduit. L’analyse syntaxique se base sur deux principes fondamentaux, l’analyse LL qui suit un cheminement de dérivation de gauche à droite et l’analyse LR qui suit le cheminement contraire, c’est à dire que les symboles les plus à droite sont dérivés en premier.
\end{abstract}
...
Afin de générer un programme cible a partir du programme source, le compilateur passe par plusieurs étapes en commençant par l'analyse lexicale jusqu'à la génération du code. Voici un schéma montrant le processus de compilation en détails:
...
Une deuxième étude sera faite afin de comparer entre l'analyse LL \S~\ref{sec:LL} et l'analyse LR \S~\ref{sec:LR} qui sont deux approches principales de l'analyse syntaxique.
A la fin, une partie sera consacrée à la démonstration de l’outil Xtext qui permet de définir différents aspects du langage traduit.  
...
De manière générale, une grammaire est définie par un quadruplet:
    ...
    N : l’ensemble des non-terminaux utilisés pour décrire les règles de production
...
Les grammaires sont analysées par des automates. Il existe plusieurs types d’analyses de grammaires qui font appel à plusieurs types d’automates.
...
Les grammaires ambiguës posent un problème lors de la compilation, c’est pour ça qu’il est préférable de les transformer en grammaire non ambiguë si c’est possible.\newline
\textit{Exemple : }\newline\newline
Prenons la grammaire suivante:  G = ( \{E\} , \{a,+,$\times$\} , R , E )

Avec R donnée par: E := E $\times$ E | E + E | a
...
L’analyse lexicale consiste à découper une chaîne de caractères en unités lexicales ou lexèmes à la demande de l’analyseur lexical.
Il est défini par un ensembles d’expressions relationnelles qui exigent certaines séquences de caractère pour former les lexèmes.
...
Une unité lexicale ou un lexème est une chaîne de caractère qui correspond à un symbole. A l’aide du processus de segmentation, on peut extraire à partir d’un flux de caractères entrant une suite d’unités lexicales. Ensuite c’est l’analyseur lexicale qui traite ces lexèmes et les rangent dans des catégories d’entités lexicales.
...
\textit{Exemple :}\newline
Reprenant l'exemple d'analyse lexicale \ref{fig:AnalyseLexicale}. L'analyse lexicale a construit une suites d'unités lexicales a partir d'une chaîne de caractères en entrée.\newline
...
L'analyseur syntaxique est un programme qui reçoit une suite d'unités lexicales de la part de l'analyseur lexicale et doit vérifier que cette suite est engendrée par la grammaire du langage décrit.
...
Donc le mot appartient au langage. Sur cet exemple c'est simple car chaque règle commence par un terminal différent ce qui facilite le choix de la règle à appliquer. Mais ce n'est pas toujours le cas, dans la plupart des cas on se trouve devant le choix entre plusieurs règles par exemple si on considère :\newline
S => aAb\newline
A => cd | c \newline
...
Pour le savoir il faut lire le caractère suivant "b", ou alors, il faut se donner la possibilité de faire des retours en arrière :
On essaye la première règle, on aboutit à un echec, alors on retourne en arrière pour essayer la deuxième règle.
...
La chaîne analysée est la même du dernier exemple \newline W=accbbadbc\newline\newline\newline
\begin{tabular}{|l|l|}
\hline
...
Le principe de l'analsye LL est simple, le symbole nom terminal le plus à gauche est toujours celui à être développé en premier et ainsi de suite jusqu'a ce que la phrase soit reconnue. 
...
Ici, on utilise Yacc sans Lex afin de comprendre le fonctionnement. Cependant, il faut définir à la main la fonction \texttt{yylex()}. Celle-ci renvoie un entier qui indique le type de token qui a été reconnu en entrée. Si on avait utilisé Lex comme scanner, cette fonction aurait déjà été définie.
...
Un autre exemple un peu plus avancé qui permet de parser un langage qui gère des catégories. L'exemple est certe scolaire mais montre bien quelques problèmes et solutions qui peuvent survenir lors de l'écriture d'une spécification.

Plus précisément, cette spécification permet de générer un interpréteur de commande qui permet de déclarer des catégories, de naviguer entre les catégories déjà déclarées et de leurs affecter un ratio.

Voici un cas d'utilisation de cet interpréteur. On y voit la déclaration de deux catégories, l'affichage des catégories existantes à l'aide de la commande \texttt{print}, la navigation d'une catégorie à une autre, et enfin l'affection d'un ratio à une catégorie.
\begin{alltt}
categorie "macat"
[Nouvelle categorie]
> Categorie : [nom: macat, ratio: 0]
categorie "autrecategorie"
[Nouvelle categorie]
> Categorie : [nom: autrecategorie, ratio: 0]
print
2 categories enregistrees
Categorie : [nom: macat, ratio: 0]
> Categorie : [nom: autrecategorie, ratio: 0]
categorie "macat"    
> Categorie : [nom: macat, ratio: 0]
if categorie = "macat" then ratio = 9. 
print
2 categories enregistrees
> Categorie : [nom: macat, ratio: 9]
Categorie : [nom: autrecategorie, ratio: 0]
\end{alltt}

Ci-dessous, la spécification founie à Yacc pour générer cet interpréteur:
...
Pour obtenir une certaines flexibilité et une meilleures gestion d’erreurs, les programmeurs préfèrent écrire leurs parseurs de descente récursive à la main, nous allons donc vous présenter dans ce chapitre un outil, qui combine flexibilité, gestion d’erreurs  et présente tous les avantages d’un générateur de parseur,cet outil est ANTLR.
\subsubsection{Définitions}
ANTLR \cite{refANTLR} est un générateur de parseur public(OPEN SOURCE), il propose plusieurs fonctionnalités qui rendent son utilisation simple, il propose des prédicats qui permettent aux programmeurs de diriger et de contrôler le parseur grâce à des expressions syntaxiques et sémantique.\newline
...
Une description ANTLR est un collection de règles et d'actions précédé par une en-tête dans laquelle l'utilisateur définit les types des attributs par exemple.
...
Chaque alternative est composée d'une liste d'éléments.Les règles définissent aussi des arguments et retournent des valeurs.\newline
Dans l'exemple suivant, il y a \textsl{n} arguments et \textsl{m} valeurs retournées:
...
ANTLR supporte l'utilisation de prédicats sémantique et syntaxique qui permet à l'utilisateur d'indiquer la validité sémantique et syntaxique d'une production, permettant ainsi à ANTLR de gérer naturellement des situations d'analyse difficile.

\subsubsection{Quelques fonctionnalités utiles aux programmeurs}

\subsubsubsection{Intégration de l'analyse syntaxique et lexicale}

Une description ANTLR contient à la fois une spécification pour l'analyse des unités lexicales et la spécification du parseur lui-même, ce qui élimine le besoin d'avoir deux fichiers différents pour chaque spécification.\newline 
En effet ANTLR extrait automatiquement une description de l'analyseur lexicale à partir de la description intégrée de ANTLR.
...
Le premier mécanisme génère automatiquement des rapports d'erreurs, désigne là où l'erreur a été détectée et qu'est ce qui était attendu.
Le second mécanisme quant à lui, fournit un framework pour y mettre les rapports d'erreurs sémantiques et syntaxiques. Notons que le premier mécanisme ne reconnait pas les erreurs sémantiques.

\subsubsubsection{Construction de l'arbre syntaxique}

Le plus souvent, les parseurs construisent une forme intermédiaire qui sera utilisées plus tard dans des phases de compilation par exemple. En utilisant quelques annotations, ANTLR peut automatiquement construire un arbre syntaxique, évitant ainsi à l'utilisateur d'appeler la construction d'un arbre à chaque fois.
Pour créer un arbre, l'utilisateur des annotations grammaticales qui indiquent le noeud père, les feuilles et aussi qui doivent être exclus de l'arbre. Les unités lexicales précédés de "!" par exemple, sont à exclure de l'arbre.Toutes les autres unités lexicales sont considérées comme des feuilles.
...
Dans cette partie, nous allons vous montrer comment utiliser ANTLR dans un IDE tel que Eclipse.
...
ANTLR génère les fichiers sources pour le lexer et le parser ( GrammaireSimpleLexer.java et GrammaireSimpleParser.java ), copiez ces deux fichiers dans le bonne endroit dans notre IDE, ensuite vous pouvez compiler.

\subsubsection{Définir notre propre grammaire}
Dans la section précédente nous, avons vu comment installer et compiler avec ANTLR, dans cette partie nous allons vous montrer comment on a créer notre propre grammaire pour calculatrice simple.
...
\item \textsl{PLUS} Unité lexicale qui contient un seule caractère : +
\item \textsl{NUMBER} Unité lexicale qui contient n'importe quel caractère compris entre 0 et 9
\item \textsl{add} Règle syntaxique qui attend exactement et dans cette ordre \textsl{NUMBER} \textsl{+} \textsl{NUMBER}, toute autre expression affichera une erreur.
\end{itemize}

\subsubsubsection{Addition répétée}
Pour écrire une expression avec plusieurs additions: 
...
Pour analyser complètement  une expression arithmétique :
...
Pour cela nous devons définir ces espaces blancs, et dire au LEXER de tolèrer ces espaces blancs :

\begin{itemize}

\item Une espace : ' '
\item Une tabulation : \verb|\t|
\item Un Retour à la ligne : \verb|\n|
\item etc...
...
\end{verbatim}
En effet, ANTLR sauvegarde deux chaînes de communications entre le LEXER et le PARSER, une chaîne cachée et une chaine par défaut. Généralement le PARSER fait appel à la chaine par défaut, nous pouvons ainsi récupérer cette chaine et la cachée :
...
\subsubsubsection{Point d'entré pour Java}
...
Si vous n'aimez pas Java vous pouvez utiliser :  
...
La différence majeure entre Yacc et ANTLR est que Yacc génére des analyseurs syntaxiques de type LALR, alors que ceux générés par ANTLR sont de type LL. C’est une différence pour plusieurs application. Par exemple :
...
ANTLR ne peut en aucun cas gérer cette grammaire telle qu’elle est. Pour ce faire, ANTLR comme tout autre analyseur syntaxique de type LL a besoin d’éliminer le récursivité à gauche avant de la traiter.
...
Xtext\cite{refXtext} est un framework pour le développement de langages de programmation et de DSL (Domain Specific programming language).
...
DSL: est un langage de programmation dont les spécifications sont à un domaine d’applications précis,la construction des langages dédiés diffère de celle d’un langage classique, le processus de développement  peut s'avérer très complexe, sa conception nécessite une double compétence sur le domaine à traiter et en développement informatique (exp: SQL destiné à interroger ou manipuler une base de données relationnelle).

\subsection{Definition Eclipse}
Eclipse\cite{refEclipse} IDE est un environnement de développement intégré libre (le terme Eclipse désigne également le projet correspondant, lancé par IBM) extensible, universel et polyvalent, permettant potentiellement de créer des projets de développement mettant en œuvre n'importe quel langage de programmation. Eclipse IDE est principalement écrit en Java (à l'aide de la bibliothèque graphique SWT, d'IBM), et ce langage, grâce à des bibliothèques spécifiques, est également utilisé pour écrire des extensions.
...
\subsection{Diagramme Xtext}

\begin{figure}[h]
	\centering
		\includegraphics[width=1.00\textwidth]{DiagrammeXtext.png}
	\caption{Diagramme de flux Xtext}
	\label{fig:DiagrammeXtext}
\end{figure}\FloatBarrier

\subsubsection{Exemple}
On va prendre l'exemple de la grammaire précédente, et on va voir comment générer un éditeur sous forme d'un plugin eclipse avec Xtext\cite{refDocXtext}.
...
Tout d'abord on va créer un projet Xtext avec comme nom du projet \texttt{org.xtext.demo},langage du projet \texttt{org.xtext.example.demo} et comme extension .demo.
...
Pour le faire, on va dans le fichier \texttt{demo.xtext} et on écrit la grammaire suivante:
...
Pour faire un test, il suffit de faire un clic droit sur le projet run as >Eclipse Application, un Eclipse va etre lancer qui a maintenant le plugin.
...
Une validation est une régle sur la grammaire (exp:un nom de categorie commence toujours par une majuscule), si l'utilisateur entre un nom en minuscule, on va le souligner.

Pour faire ceci, on va créer dans le package \texttt{org.xtext.example.validation} géneré par Eclipse une classe qu'on va appeler DemoValidator et qui va contenir la méthode checkCategory qui va faire un test sur la première lettre du nom de la categorie et qui surligne le nom si cette lettre est en minuscule.
